...
Running dots indicate a live application process. Icons dim when the application has no open windows across all desktops; minimized and hidden windows count as open. Dimming affects the icon only, so running dots and notification badges stay clear. A pinned application remains after quitting with its dot off and icon dimmed; an unpinned application disappears after quitting. Settings → Icons offers **Running dots** or an additional **Box running** style; **Advanced → Dim apps without windows** controls icon opacity for both styles. Existing saved indicator and opacity values are preserved.
