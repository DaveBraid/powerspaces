...
- **Icons:** icon size/spacing, running dots and optional running-app outlines; dimming applies to apps without open windows.
...
Static window previews are opt-in under **Settings → Effects → Window previews**. After enabling the switch, use its separate **Screen Recording** permission button. Leaving previews off requires no Screen Recording permission. Accessibility remains a separate permission for focusing the selected window.

Hover for about 350 ms to see the app's windows belonging to this dock's desktop/display; click a thumbnail to focus that exact window. Move into the preview to keep it open; leaving both regions closes it after about 300 ms. The shared full-screen section previews its selected window even on another desktop, without switching desktops or activating the app. Each opening captures windows once using ScreenCaptureKit (on macOS 27, offscreen full-screen capture failures fall back to the dynamically detected legacy single-window screenshot API under the same Screen Recording permission), with one capture in flight and bounded image dimensions. Images stay in memory, are explicitly cleared on close, and are never saved or uploaded. Minimized/hidden or unavailable windows show placeholders; clicking revalidates desktop ownership before restoring and focusing. No live refresh or window-close controls are included.


Unpinned full-screen windows now appear individually in a shared section at the trailing end of every PS dock (bottom when docked vertically). Its divider appears only while the section is nonempty and uses the existing divider size/spacing controls. Apps pinned on any desktop remain under their existing pin rules and do not enter this shared section. Ordinary windows keep their desktop isolation; entering/exiting full screen or closing the last full-screen window updates the section automatically. Clicking a shared item goes to that exact full-screen window after revalidation; it does not reopen the app or minimize the window. The shared section uses stable window order and does not change saved application pin order.

Running dots indicate a live application process. Icons dim when the application has no open windows across all desktops; minimized and hidden windows count as open; invisible backing objects left after closing a window do not. Dimming affects the icon only, so running dots and notification badges stay clear. A pinned application remains after quitting with its dot off and icon dimmed; an unpinned application disappears after quitting. Settings → Icons offers **Running dots** or an additional **Box running** style; **Advanced → Dim apps without windows** controls icon opacity for both styles. Existing saved indicator and opacity values are preserved.
