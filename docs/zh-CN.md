...
共享全屏窗口分区读取所有显示器的全屏 Space，不仅检查当前可见桌面；已固定应用排除于该分区，剩余窗口尾部展示并去重。程序运行状态和全局窗口存在状态独立，设置与具体规则见[中文 README](../README.zh-CN.md#共享全屏窗口分区与状态)。

共享全屏分区也支持静态缩略图，悬停直接截取对应全屏窗口，点击才切换到它；沿用独立屏幕录制授权及内存内单次捕获。

`--check-fullscreen-preview` 仅使用独立测试应用 `local.ps.fullscreen-validation`，验证离屏全屏截图的内容、非激活行为及点击精确聚焦；结束后关闭测试应用。其他桌面全屏窗口的 AX 补查当前限于已验证的 macOS 27.0 / 26A428，系统能力失效时会提示，避免切到错误窗口。
